  ...
  useEffect(() => {
    if (userData && courseData) {
      setIsAlreadyEnrolled(
        userData.enrolledCourses.some((id) => String(id) === String(courseData._id))
      )
    }
  }, [userData, courseData])
              ...
              {courseData.courseContent.map((chapter, index) => (
                <div key={index} className="border border-gray-300 bg-white mb-2 rounded">
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? 'max-h-[500px]' : 'max-h-0'}`}>
                    <ul className="list-none pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300 space-y-1">
                      {chapter.chapterContent.map((lecture, i) => (
                        <li key={i} className="flex items-center gap-2 py-1">
                          <img src={assets.play_icon} alt="play icon" className="w-4 h-4 shrink-0" />
                          <div className="flex items-center justify-between w-full text-gray-800 text-xs md:text-sm gap-2">

                            <div className="flex items-center gap-2 flex-1 min-w-0">
                              <p className="truncate">{lecture.lectureTitle}</p>

                              {lecture.isPreviewFree && (
                                <span className="shrink-0 text-xs bg-green-100 text-green-700 border border-green-300 px-1.5 py-0.5 rounded-full font-medium">
                                  🔓 Free
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              {lecture.isPreviewFree && (
                                <span
                                  ...
                                  className="text-xs bg-blue-600 text-white px-2 py-0.5 rounded cursor-pointer hover:bg-blue-700 transition-colors select-none"
                                >
                                  Preview
                                </span>
                              )}
        ...
        <div className="max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
          {playerData
            ? (extractYoutubeVideoId(playerData.lectureUrl) ? (
                ...
                <div className="aspect-video w-full flex items-center justify-center bg-gray-100 text-gray-600 text-xs px-3 text-center">
                  Add a valid YouTube URL for this lecture (e.g. youtube.com/watch?v=…).
                </div>
              ))
